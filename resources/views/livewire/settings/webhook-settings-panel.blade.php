<div>
    ...
    @if($endpoints->isEmpty())
        <p class="text-sm text-gray-500">No webhook endpoints configured. Add one to receive notifications for experiment and project events.</p>
    @else
        <div class="space-y-3">
                ...
                <div class="flex items-center justify-between rounded-lg border border-gray-200 p-3">
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium {{ $endpoint->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' }}">
                                {{ $endpoint->is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </div>
                        <p class="mt-0.5 truncate text-sm text-gray-500">{{ $endpoint->url }}</p>
                    ...
                    <div class="ml-4 flex items-center gap-2">
                        <button wire:click="toggleActive('{{ $endpoint->id }}')" class="text-sm text-gray-500 hover:text-gray-700">
    ...
    @if($showForm)
        <div class="mt-4 rounded-lg border border-blue-200 bg-blue-50 p-4">
            <h4 class="mb-3 font-medium text-gray-900">{{ $editingId ? 'Edit Endpoint' : 'New Endpoint' }}</h4>
            <div class="space-y-3">
                ...
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700">Events</label>
                    <div class="grid grid-cols-2 gap-2">
                        @foreach($availableEvents as $event)
                            <label class="flex items-center gap-2 text-sm">
                                <input type="checkbox" wire:model="selectedEvents" value="{{ $event->value }}" class="rounded border-gray-300 text-blue-600">
                                {{ $event->label() }}
                            </label>
                        @endforeach
                    </div>
                ...
                <div class="flex gap-2">
                    ...
                    <button wire:click="$set('showForm', false)" class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                        Cancel
                    </button>
                </div>
            </div>
